...
public class matchescontroller implements BaseController {
    ...
    private void populateUserVBox() {
        ...
        List<UserInfo> users = DatabaseManager.getNonFriendUsers(userId);
    ...
    private Button createSendRequestButton(int receiverId) {
        ...
        sendRequestButton.setOnAction(event -> {
            ...
            HBox hboxToRemove = (HBox) sendRequestButton.getParent();
            userVBox.getChildren().remove(hboxToRemove);

            boolean success = DatabaseManager.sendFriendRequest(userId, receiverIdFromButton);
